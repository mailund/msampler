...
public:
  ...
  using NonNegative = range::NonNegative;
  using Unit = range::Unit;
  ...
  MultinomialSampler(std::initializer_list<NonNegative> weights);
  ...
  Unit outcome_probability(Outcome i) const { return initial_probs_[i]; }
  ...
  Unit sample_probability(int i) const;
  ...
  Outcome sample(Unit u0, Unit u1) const {
